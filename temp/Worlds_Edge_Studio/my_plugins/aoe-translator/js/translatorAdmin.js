    ...
    init: function() {
		...
		function fetchTranslations() {
			...
			function altSuccessStep() {
				...
				alt_post('pva_create', JSON.stringify(newOtherThing));
			}

			function rebuildByLang() {
				//console.log('otherThing ', otherThing);
				otherThing.forEach(function(oThings) {
					oThings.forEach(function(oThing) {
						
						var existing = newOtherThing.filter(function(v,i) {
							return v.to == oThing.to;
						});
						
						if(existing.length) {
							var existingIndex = newOtherThing.indexOf(existing[0]);
							var entries = Object.entries(oThing);
							newOtherThing[existingIndex][entries[1][0]] = entries[1][1];
							
						} else {
							newOtherThing.push(oThing);
						}
					});
				});
				altSuccessStep();
			...
			//console.log('requested_fields ', requested_fields);

			

			//console.log(JSON.stringify(requested_fields));

			//let string_fields = JSON.stringify(requested_fields);
			...
			var altproxy = 'https://aqueous-sea-04377.herokuapp.com/';
			var secondproxy = 'https://blooming-shore-59830.herokuapp.com/';
			...
			for(let translation_loop = 0; translation_loop < group_to_trans.length; translation_loop++) {
				...
				//console.log('translation_loop ', translation_loop);
							...
							for(r = 0; r < data['responseText'].length; r++) {
								if (data['responseText'][r].hasOwnProperty("text")) {
									...
									if(translation_loop == 0) {
							    		...
							    		otherThing.push(data['responseText']);
							    	} else if(translation_loop == 1) {
							    		data['responseText'][r].content = data['responseText'][r].text;
							    		otherThing.push(data['responseText']);
							    	} else {
							    		//data['responseText'][r].fields = data['responseText'][r].text;
							    		//otherThing.push(data['responseText']);
							    		// third loop, pull key from field key collection
							    		extra_lang_to.push(saved_keys[translation_loop]);
							    		this_key = [saved_keys[translation_loop]];
