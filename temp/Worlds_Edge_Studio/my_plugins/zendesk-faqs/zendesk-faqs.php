<?php /**
 * Plugin Name: Zendesk FAQ
 * Plugin URI: na
 * Description: Allow Zendesk FAQ selection in Admin menu.
 * Version: 1.0
 * Author: Heather Sullivan
 */

add_action('admin_menu', 'my_admin_zendesk_faq_menu');
add_action('admin_enqueue_scripts', 'my_admin_zendesk_add_js_file');

function my_admin_zendesk_faq_menu() {
    ...
    add_action( 'admin_init', 'update_zendesk_faqs' );
}


function my_admin_zendesk_add_js_file() {
    wp_enqueue_script( 
        'zendesk-faqs-plugin',
        plugins_url( '/js/zenFaqsAdmin.js', __FILE__ ),
        array(), '1.2', true
    );

    wp_register_style('myzendeskstylesheet', '/wp-content/plugins/zendesk-faqs/styles/style.css');
    wp_enqueue_style('myzendeskstylesheet');
}

if( !function_exists("update_zendesk_faqs") ) { 
    function update_zendesk_faqs() {   
...
function my_admin_zendesk_faq_contents() { ?>
