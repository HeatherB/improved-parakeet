...
//  learnByPoints() {
  ...
  //  buildLearnToPlay();
    ...
    */
    ...
    _backalt_post(dataAction,dataString) {
      /*jQuery(document).ready(function($) {
        var data = {
          'action': dataAction,
          'transData': dataString,
      ...
      makeAnchorsFrom.forEach(function(makeAnchor) {
        ...
        if(isMap) {
          ...
          let winWidth = window.outerWidth;
            ...
            //anchor.onclick = self._loadMapPoint;
            ...
            //button.onclick = self._loadMapPoint;
        ...
        if(isVideo) {
          let timestamp = makeAnchor.closest('li').getAttribute('data-timestamp');
              ...
              anchor.onclick = self._loadVideoPoint;
    ...
    _activateEventListeners() {
      ...
      //document.body.onclick = () => this._loadMapPoint();

      mapTriggers.forEach(mapTrigger => {
        //mapTrigger.addEventListener('click', self._loadMapPoint);
        mapTrigger.onclick = () => self._loadMapPoint();
      });
      nextBTNs.forEach(nextBTN => {
        //nextBTN.addEventListener('click', self._navigate);
        nextBTN.onclick = () => self._navigate();
      });
      prevBTNs.forEach(prevBTN => {
        //prevBTN.addEventListener('click', self._navigate);
        prevBTN.onclick = () => self._navigate();
      });
      zoomBTNs.forEach(zoomBTN => {
        //zoomBTN.addEventListener('click', self._loadMapPoint);
        zoomBTN.onclick = () => self._loadMapPoint();
      });
      scrollDownBTNS.forEach(scrollDBTN => {
        //scrollDBTN.addEventListener('click', self._scrollToNextSection);
        scrollDBTN.onclick = () => self._scrollToNextSection();
      });
      scrollUpBTNS.forEach(scrollUBTN => {
        //scrollUBTN.addEventListener('click', self._scrollToPrevSection);
