...
class GamesController{
  ...
  public function home_games(){
    ...
    $data = [];
    $temp = [];
    if( have_rows('featured_games') ):
      while( have_rows('featured_games') ): the_row();
        foreach ($wp_fields as $field) {
          $post = get_sub_field('game');
          $temp[$field] = call_user_func_array(array($this, $field), array($post->ID));
        };
  ...
  function excerpt($post_id) {
    ...
    $line = preg_replace_callback("/(&#[0-9]+;)/", function($m) { return mb_convert_encoding($m[1], "UTF-8", "HTML-ENTITIES"); }, $excerpt);
