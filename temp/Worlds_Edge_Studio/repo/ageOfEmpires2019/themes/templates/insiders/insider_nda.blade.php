<?php
    ...
    $greeting = str_replace('[username]','<span id="value_gamertag">' . $user->msa_modern_gamertag ?? $user->msa_gt . '</span>',$greeting);
                ...
                <form id="insider_nda">
                    ...
                    <ul class="insider_settings_cb not_a_list">
                        @foreach($agreements as $index => $question)
                            <li>
                                <input type="checkbox" class="standard required" name="{{$agreement_values[$index]}}" id="{{$agreement_values[$index]}}" data-q_value="{{$agreement_values[$index]}}" />
