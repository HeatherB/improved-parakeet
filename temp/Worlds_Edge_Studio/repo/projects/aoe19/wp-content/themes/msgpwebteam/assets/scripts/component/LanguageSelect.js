...
export default class LanguageSelect {
	...
	init() {
    this.Languages = ["es", "br", "fr", "de", "it", "in", "jp", "kr", "ru", "vn", "cn", "tw"];
		...
		if (params.lang == null || params.lang == "") {			
			if (window.wp_object.locale != null && window.wp_object.locale != "") {
				...
				if ( this.Languages.includes(lang.toLowerCase())) {  				
					location.href = "/support/"+lang.toLowerCase()+"?lang="+lang.toLowerCase();
				}
			}			
    }
    this.ui = {
	...
	_onChange(e) {
		...
		if(lang.toLowerCase() == "en") {
			location.href = "/support?lang="+lang.toLowerCase();
