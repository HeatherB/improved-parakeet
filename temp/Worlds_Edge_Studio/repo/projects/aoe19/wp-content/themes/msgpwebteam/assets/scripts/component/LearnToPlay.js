...
export default class LearnToPlay {
  ...
  _alt_post(dataAction,dataString) {
    ...
    _markSectionComplete() {
      let self = this;
      let thisContentSection = event.target.closest('.js-ltpSection');
      let progressSection = thisContentSection.querySelector('.lesson_heading').getAttribute('id');
      let progressPoint = thisContentSection.querySelector('.lesson_heading').innerHTML.trim();
      thisContentSection.querySelector('.js-mark_self_complete').dataset.completed = 1;
    ...
    _navigate() {
      ...
      if(isMap) {
        ...
        btns = thisMap.querySelectorAll('.js-mapWrapper .js-mapTrigger');
        items = contentList.querySelectorAll('li.pointsWrapperItem');
        amount = items.length;
        mapX = 0;
        mapY = 0;
    ...
    _oneMoreMade() {
      let self = this;
      self.ui.madeMapsAndVideosAndSlides++;
      ...
      let mapsAndVideosAndSlides = ltpMaps.length + ltpVideos.length + ltpSlides.length;
      if(self.ui.madeMapsAndVideosAndSlides === mapsAndVideosAndSlides) {
    ...
    _makeAnchors(anchorPointsWrapper) {
      ...
      let isVideo = false;
      let isSlide = false;
      let anchorList = anchorPointsWrapper.parentNode.querySelector('.js-alphaQuicknav');
      if(anchorPointsWrapper.classList.contains('js-mapWrapper')) {
        isMap = true;
      } else if(anchorPointsWrapper.classList.contains('js-mapVideoWrapper')) {
        isVideo = true; //js-mapVideoWrapper //js-ltpVideo
        anchorList = anchorPointsWrapper.parentNode.parentNode.querySelector('.js-alphaQuicknav');
    ...
    _buildProgress(ltpVideo,videoDuration) {
      ...
      let thisVideo = ltpVideo.querySelector('video');
    ...
    _activateEventListeners() {
      ...
      let altVideoTriggers = document.querySelectorAll('.js-alphaQuicknav');
      let mapTriggers = document.querySelectorAll('.js-mapTrigger');
      let slideTriggers = document.querySelectorAll('.js-slideTrigger');
      let completionTriggers = document.querySelectorAll('.js-mark_self_complete');
      ...
      altVideoTriggers.forEach(altVideoTrigger => {
        altVideoTrigger.addEventListener('click', (event) => {
          if(event.target.parentNode.classList.contains('js-videoTrigger')) {
            self._loadVideoPoint();
          }
        });
      });
      ...
      completionTriggers.forEach(completionTrigger => {
        completionTrigger.onclick = () => self._markSectionComplete();
      });
    ...
    _buildLearnToPlay() {
      ...
      let ltp_group_nav_hrefs = document.querySelectorAll('.lesson_groups_nav a');
      let ltp_related_lesson_hrefs = document.querySelectorAll('.related-lessons a');
      let rl_btn_group = [];

      ltp_related_lesson_hrefs.forEach(function(rl_href) {
        let just_href = rl_href.getAttribute('href');
        let split_ref = just_href.split('/');
        split_ref.pop();
        rl_btn_group.push(split_ref[split_ref.length-1]);
      });

      let btn_1 = document.querySelectorAll('.lesson_groups_nav a')[0].getAttribute('href');
      let btn_2 = document.querySelectorAll('.lesson_groups_nav a')[1].getAttribute('href');

      let name_intersection_1 = rl_btn_group.filter(element => btn_1.includes(element));
      let name_intersection_2 = rl_btn_group.filter(element => btn_2.includes(element));

      if(name_intersection_1.length > 0) {
        document.querySelectorAll('.lesson_groups_nav a')[0].classList.add('active');
      }

      if(name_intersection_2.length > 0) {
        document.querySelectorAll('.lesson_groups_nav a')[1].classList.add('active');
      }
      ...
      if(ltpVideos) {
        ltpVideos.forEach(function(ltpVideo) {
          ltpVideo.querySelector('.js-ltpVideo').addEventListener( "loadedmetadata", function () {
            self._makeAnchors(ltpVideo);
            self._buildVideoCanvas(ltpVideo);
          });
        });
      }
