...
export default class ModForm {
  ...
  _getModData() {
    let xhrs = [
      ajaxGet({ url: config.api.games }), // get mod types for select input
      ajaxGet({ url: config.api.modsTypes }), // get mod types for select input
      ajaxGet({ url: config.api.modsTags }), // get mod tags for select input
    ];     

    // get mod data if we're in edit mode
    if (this.modId) {
      xhrs.push(ajaxPost({
        url: config.api.modsDetail,
        data: JSON.stringify({
          modid: this.modId,
        }),
      }));
  ...
  _initTinyMCE(selector) {
    $(selector).parent().addClass('form__wysiwyg');
    tinymce.init({
      selector: selector,
      theme: 'modern',
      height: 250,
  ...
  _initDropzones(existingImages) {
    ...
    this.dropzoneMod = new Dropzone('#dropzone-mod', Object.assign({
      ...
      dictDefaultMessage: 'Drop zip file here, or click to attach.',
  ...
  _submitForm() {
    ...
    let apiUrl = this.modId === null ? config.api.modsCreate : config.api.modsEdit;
    // reset form
    this.ui.$form.removeClass('error');
    this.loader.show();
    let selectedTags = [];
