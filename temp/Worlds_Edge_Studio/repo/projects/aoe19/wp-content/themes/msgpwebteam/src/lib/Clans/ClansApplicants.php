...
class ClansApplicants
{
  ...
  {
    add_action( 'wp_ajax_nopriv_applicants', [$this, 'get_applicants'] );
    add_action( 'wp_ajax_applicants', [$this, 'get_applicants'] );
  }
  ...
  public function get_applicants() {
    ...
    $default_args = array(
      'meta_query' => array(
        "clan_applications" => array(
          "key" => "clan_applications",
          "compare" => "EXISTS"
        ),
      )
    );
    ...
    if ( ! empty( $members_query->results ) ) {
      foreach ( $members_query->results as $member ) {
        ...
        $data['totalCount'] = sizeof($data['playersList']);
      }
    }
