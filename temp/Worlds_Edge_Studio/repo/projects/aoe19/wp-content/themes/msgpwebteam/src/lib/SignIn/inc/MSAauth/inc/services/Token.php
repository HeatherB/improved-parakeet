...
class Token
{
    ...
    static public function process_token()
    {
        if (isset($_GET["user_code"])) {
            $redis = new \Redis();
            $redisAuth = class_exists('\Roots\AzureResource\AzureResource',false) ? \Roots\AzureResource\AzureResource::get_key('RedisConnectionAuth') : '';
            ...
            $_SESSION['age_login_expire'] = time() + 14400;
