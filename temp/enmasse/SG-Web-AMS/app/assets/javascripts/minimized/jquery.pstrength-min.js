...
digitalspaghetti.password = {
  ...
  'attachWidget': function (element) {
    var output = ['<div id="password-strength" class="pstrength-meter-cont">'];
    if (digitalspaghetti.password.options.displayMinChar && !digitalspaghetti.password.tooShort) {
      output.push('<div class="pstrength-minchar">' + digitalspaghetti.password.options.minCharText.replace('%d', digitalspaghetti.password.options.minChar) + '</div>');
    }
    output.push('<div class="pstrength-bar" style="border: 1px solid white; font-size: 1px; height: 10px;"></div>');
    output.push('<div class="pstrength-info"></div>');
    output.push('</div>');
    output = output.join('');
    jQuery(".pstrength-meter-cont").replaceWith(output);
  },
  ...
  'calculateScore': function (word) {
    ...
    for (var key in digitalspaghetti.password.rules) if (digitalspaghetti.password.rules.hasOwnProperty(key)) {
      ...
      if (digitalspaghetti.password.totalscore <= digitalspaghetti.password.options.scores[0]) {
        ...
        digitalspaghetti.password.width =  "92";
      }
      jQuery('.pstrength-bar').stop();

      if (digitalspaghetti.password.options.displayMinChar && !digitalspaghetti.password.tooShort) {
        jQuery('.pstrength-minchar').hide();
      } else {
        jQuery('.pstrength-minchar').show();
      }

      jQuery('.pstrength-bar').animate({opacity: 0.5}, 'fast', 'linear', function () {
        jQuery(this).css({'display': 'block', 'background-color': digitalspaghetti.password.strColor, 'width': digitalspaghetti.password.width + "%"});
        jQuery(this).animate({opacity: 1}, 'fast', 'linear');
      });
      jQuery('.pstrength-info').text(digitalspaghetti.password.strText)
    }
  }
};
