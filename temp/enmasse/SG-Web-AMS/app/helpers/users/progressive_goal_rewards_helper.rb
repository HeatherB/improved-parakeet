module Users::ProgressiveGoalRewardsHelper
  include UsersHelper

  def reward_sku_status_text(promotion, reward, sku, unredeemed_arr)
    out = ""
    if unredeemed_arr.nil? || unredeemed_arr.empty?
      ...
      if (arr = status_hash[AssetFulfillment.fulfillment_status_for(:fail_fatal)]).present?
        ...
        out << "<form id='game-code-info-form' action='#{submit_required_info_users_progressive_goal_reward_path(reward)}' data-div='redeemed-code' method='post'>"
        out << "<input type='hidden' name='authenticity_token' value='#{form_authenticity_token}' />"
        has_items = false
        fields.each do |f|
          f_out, f_has_items = reward_html_for_required_input(f, sku)
          out << f_out
  ...
  def reward_html_for_required_input(field, sku)
    out, has_items = "", false
    case field.to_s
