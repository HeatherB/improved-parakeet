...
class UserCancelNotification < LogAR
  ...
  def self.create_initial!(request, user, sub, server_ip="")
    create!(
      :payment_method      => "user_cancel",
      :request_type        => "cancel",
      :subscription_id     => sub.id,
      :user_id             => user.id,
      :ext_product_id      => sub.ext_product_id,
      :ext_user_id         => sub.ext_user_id,
      :ext_subscription_id => sub.ext_subscription_id,
      :duration            => 0,
      :recurring           => false,
      :request_ip          => request.remote_ip,
      :server_ip           => server_ip,
      :notification_json   => sub.to_json,
      :has_errors          => true
    )
  end
