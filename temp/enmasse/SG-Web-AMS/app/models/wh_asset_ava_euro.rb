...
class WhAssetAVAEuro < WarehouseSkuAsset
    ...
    write_log(asset_fulfillment) do |log|
      self.class.transaction(:requires_new => true) do
        begin
          ...
          log << "Attempting to call send_euros(#{acct.id}, #{amount})"
          res = GameTools::AVA::API.new.send_euros(acct.id, amount)
          log << "Response: #{res}"
          case res['error_code']
          when 'not_found'
            log << "Failed, error_message: #{res['message']}"
            raise RuntimeError.new("send_euros failed, error_emssage: #{res['message']}")
          when 'operation_error'
            log << "Failed, error_message: #{res['message']}"
            raise RuntimeError.new("send_euros failed, error_message: #{res['message']}")
          when nil
            log << 'Success'
          end
        rescue Exception => e
