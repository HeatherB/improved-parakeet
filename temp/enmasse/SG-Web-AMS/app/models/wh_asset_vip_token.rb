...
class WhAssetVipToken < WarehouseSkuAsset
    ...
    write_log(asset_fulfillment) do |log|
      self.class.transaction(:requires_new => true) do
        begin
          ...
          begin
            add_vip_token_to_account(acct, meta_hash, asset_fulfillment)
          rescue Exception => e
  ...
  def add_vip_token_to_account(acct, meta_hash, asset_fulfillment)
    additional_info = {
      source_type: asset_fulfillment.class.name,
      source_id:   asset_fulfillment.id,
      meta_hash:   meta_hash,
    }
    message = meta_hash['message'] || 'WhAssetVipToken'
    acct.add_vip_token(meta_hash['vip_token'].to_i, external_transaction_id: asset_fulfillment.id, message: message, additional_info: additional_info)
  end
