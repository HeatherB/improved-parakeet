$.widget( "eme.lightbox", {
  ...
  initLightbox: function() {
    ...
    function closeLightBox(closeThis) {
      ...
      //$('.emelightbox-pop').find('.eme_slide').append($('.emelightbox-pop').find('.eme_slide').data('flipbook'));
      $('.emelightbox-pop').find('.carouselpager').removeClass('active');
     // $('.emelightbox-pop').find('.eme_slide').attr('data-flipbook', '');
...
$(document).on('click', '.emelightbox-pop .eme_slide', function() {
  ...
  /* special for flipbook */
//  $(this).attr('data-flipbook', $(this).html());
  
  
  var emelightboxblock = '<div id="emelightbox-wrapper"><div id="emelightbox" style="top:' + offsetTop + 'px;"><div class="emelightbox-close"><span>X</span></div><div id="emelightbox-content">' + emelightboxContent + '</div></div></div>';
 ...
 /* for image slides and video slides */
  ...
  loadedLB.each(function(i) {
    //var toLoadArr = $(this).data('arr').split(",");
    var toLoadSlide = $(this).data('large');
    var toLoadVideo = $(this).data('video');
    var toLoadContent = $(this).data('content');
    /*if(toLoadArr) {
      $(toLoadArr).each(function(thisArr) {
        console.log('thisArr ', toLoadArr[thisArr]);
      });
    }*/
    if(toLoadSlide) {
    ...
    if(toLoadContent) {
      var useThis = $('body').find('#' + toLoadContent).clone();
      useThis.css("display","block");
      $(this).replaceWith(useThis);
    }
...
$(document).on('click', '#emelightbox-content .next', function (event) {
  event.preventDefault();
  var activatedGallery = $('#emelightbox');
  navigate(1, activatedGallery);
});
$(document).on('click', '#emelightbox-content .prev', function (event) {
  event.preventDefault();
  var activatedGallery = $('#emelightbox');
  navigate(-1, activatedGallery);
});
