...
var BrawlerClassApp = {
  ...
  init: function(settings) {
    var s = BrawlerClassApp.s;
    s = $.extend(s, settings);
    BrawlerClassApp.bindEvents();
  },
  bindEvents: function() {
    var s = BrawlerClassApp.s;
    // set up the scrolling menu waypoint
    $(window).waypoint(function() {
      BrawlerClassApp.toggleMainNav();
    }, {offset: s.navOffset});
    // set up new world toggle waypoint
    $(s.intro).waypoint(function(direction) {
      BrawlerClassApp.toggleSelectedMenuItem('#intro', direction);
      // only record event on scroll and not click
      if(s.navnotscroll == false) {
        BrawlerClassApp.triggerGAEvent('Reached Page Section', 'Hero top of Page');
      }
      //if(direction == 'up') {
      //  BrawlerClassApp.moveBarLeft();
      //}
    $('#main-nav a').removeClass('active');
    });
    // set up Brawler class toggle waypoint
    $(s.highlights).waypoint(function(direction) {
      BrawlerClassApp.toggleSelectedMenuItem('#highlights', direction);
      // only record event on scroll and not click
      if(s.navnotscroll == false) {
        BrawlerClassApp.triggerGAEvent('Reached Page Section', 'Class Highlights');
      }
         //BrawlerClassApp.moveBarRight();
      $('#main-nav a').removeClass('active');
      $('#main-nav a#nav-highlights').toggleClass('active');
    });
    $(s.skillvideos).waypoint(function(direction) {
      BrawlerClassApp.toggleSelectedMenuItem('#skillvideos', direction);
      if(s.navnotscroll == false) {
        BrawlerClassApp.triggerGAEvent('Reached Page Section', 'Skill Videos');
      }
    });
    $(s.skills).waypoint(function(direction) {
      BrawlerClassApp.toggleSelectedMenuItem('#skills', direction);
      // only record event on scroll and not click
      if(s.navnotscroll == false) {
        BrawlerClassApp.triggerGAEvent('Reached Page Section', 'Skills');
      }
      $('#main-nav a').removeClass('active');
      $('#main-nav a#nav-skills').toggleClass('active');
    });
    $(s.origin).waypoint(function(direction) {
      BrawlerClassApp.toggleSelectedMenuItem('#origin', direction);
      // only record event on scroll and not click
      if(s.navnotscroll == false) {
        BrawlerClassApp.triggerGAEvent('Reached Page Section', 'Origin');
      }
      $('#main-nav a').removeClass('active');
      $('#main-nav a#nav-origin').toggleClass('active');
    });
    $(s.ankle).waypoint(function(direction) {
      BrawlerClassApp.toggleSelectedMenuItem('.ankle', direction);
      BrawlerClassApp.triggerGAEvent('Reached Page Section', 'Ankle');
    });
    // set up footer toggle waypoint
    $(s.close).waypoint(function(direction) {
      BrawlerClassApp.toggleSelectedMenuItem('#close', direction);
      BrawlerClassApp.triggerGAEvent('Reached Page Section', 'Ankle CTA');
    });
    // custom scroller
    $(s.skillCheck).on('click', function() {
      $($(this).attr('href')).velocity("scroll", {duration: 1000, easing: 'ease-in-out', offset: -54});
      BrawlerClassApp.triggerGAEvent('Reached Page Section', 'Fill Skills Listing');
    });
    // set up click listeners on the menu buttons 
    s.nav.on('click', 'a', BrawlerClassApp.handleNavClick);
    s.vid.on('click', BrawlerClassApp.handleVideoClick);
    s.btnPlay.on('click', BrawlerClassApp.handlePlayClick);
    //new
    s.shareFacebook.on('click', BrawlerClassApp.handleFacebookShareClick);
    s.shareTwitter.on('click', BrawlerClassApp.handleTwitterShareClick);
    //end
    $('.cta-sign-up').on('click', BrawlerClassApp.handleSignUpClick);
    BrawlerClassApp.createPageTimers();
    BrawlerClassApp.setUpKonamiCode();
  },
  handlePlayClick: function(e) {
    e.stopPropagation();
    BrawlerClassApp.triggerGAEvent('Clicked Button', 'Play for FREE Button');
  },
  handleSignUpClick: function(e) {
    e.stopPropagation();
    BrawlerClassApp.triggerGAEvent('Clicked Link', 'Sign up now');
  },
  /* new tracking events */
  handleFacebookShareClick: function(e) {
    e.stopPropagation();
    BrawlerClassApp.triggerGAEvent('Clicked Button', 'Share This Facebook Button');
  },
  handleTwitterShareClick: function(e) {
    e.stopPropagation();
    BrawlerClassApp.triggerGAEvent('Clicked Button', 'Share This Twitter Button');
  },
  /* end new tracking events */
  createPageTimers: function() {
    // 1 minute on page
    setTimeout(function() {
      BrawlerClassApp.triggerGAEvent('Spent 1 minute on page', ' ');
    }, 1 * 60 * 1000);
    // 2 minutes on page
    setTimeout(function() {
      BrawlerClassApp.triggerGAEvent('Spent 2 minutes on page', ' ');
    }, 2 * 60 * 1000);
    // 5 minutes on page
    setTimeout(function() {
      BrawlerClassApp.triggerGAEvent('Spent 5 minutes on page', ' ');
    }, 5 * 60 * 1000);
  },
  setUpKonamiCode: function() {
    // BrawlerClassApp.s.easter_egg = new Konami(function() {BrawlerClassApp.createParticleSystem()});
  },
  ...
  handleVideoClick: function(e) {
    ...
    BrawlerClassApp.triggerGAEvent('Clicked Button', 'Video Play Button ' + this.id);
    e.preventDefault();
  },
  ...
  handleNavClick: function(e) {
    ...
    if (external) {
      if (target == '/') {
        BrawlerClassApp.triggerGAEvent('Clicked Button', 'Back to Tera - Menu');
      } else {
        BrawlerClassApp.triggerGAEvent('Clicked Button', 'Get Started - Menu');
      }
    } else {
      e.preventDefault();
      ...
      complete: function() {
        ...
        BrawlerClassApp.triggerGAEvent('Clicked Internal Navigation - ', 'section (' + target + ')' );
        s.navnotscroll = false;
      }});
    }
    //var fromLink = $(this).attr('id')
    //BrawlerClassApp.moveBarWhere(fromLink);
  },
  ...
  triggerGAEvent: function(action, label ) {
    ...
    if (_gaq) {
      if (label != ' ') {
        _gaq.push(['_trackEvent', 'Landing Pages', action, BrawlerClassApp.s.url + ' - ' + label]);
      } else {
        _gaq.push(['_trackEvent', 'Landing Pages', action, BrawlerClassApp.s.url]);
      }
    }
  }
}
