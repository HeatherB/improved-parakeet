...
var NinjaClassApp = {
  ...
  init: function(settings) {
    var s = NinjaClassApp.s;
    s = $.extend(s, settings);
    NinjaClassApp.bindEvents();
  },
  bindEvents: function() {
    var s = NinjaClassApp.s;
    // set up the scrolling menu waypoint
    $(window).waypoint(function() {
      NinjaClassApp.toggleMainNav();
    }, {offset: s.navOffset});
    // set up new world toggle waypoint
    $(s.intro).waypoint(function(direction) {
      NinjaClassApp.toggleSelectedMenuItem('#intro', direction);
      // only record event on scroll and not click
      if(s.navnotscroll == false) {
        NinjaClassApp.triggerGAEvent('Reached Page Section', 'Hero top of Page');
      }
    $('#main-nav a').removeClass('active');
    });
    // set up Brawler class toggle waypoint
    $(s.highlights).waypoint(function(direction) {
      NinjaClassApp.toggleSelectedMenuItem('#highlights', direction);
      // only record event on scroll and not click
      if(s.navnotscroll == false) {
        NinjaClassApp.triggerGAEvent('Reached Page Section', 'Class Overview');
      }
         //NinjaClassApp.moveBarRight();
      $('#main-nav a').removeClass('active');
      $('#main-nav a#nav-highlights').toggleClass('active');
    });
    $(s.guide).waypoint(function(direction) {
      NinjaClassApp.toggleSelectedMenuItem('#guide', direction);
      // only record event on scroll and not click
      if(s.navnotscroll == false) {
        NinjaClassApp.triggerGAEvent('Reached Page Section', 'Video Guide');
      }
      $('#main-nav a').removeClass('active');
      $('#main-nav a#nav-guide').toggleClass('active');
    });
    $(s.skills).waypoint(function(direction) {
      NinjaClassApp.toggleSelectedMenuItem('#skills', direction);
      // only record event on scroll and not click
      if(s.navnotscroll == false) {
        NinjaClassApp.triggerGAEvent('Reached Page Section', 'Skills and Glyphs');
      }
      $('#main-nav a').removeClass('active');
      $('#main-nav a#nav-skills').toggleClass('active');
    });
    $(s.footer).waypoint(function(direction) {
      NinjaClassApp.triggerGAEvent('Reached Page Section', 'Footer with CTA');
    });
    $(s.ankle).waypoint(function(direction) {
      NinjaClassApp.toggleSelectedMenuItem('.ankle', direction);
      NinjaClassApp.triggerGAEvent('Reached Page Section', 'Ankle');
    });
    // set up click listeners on the menu buttons 
    s.nav.on('click', 'a', NinjaClassApp.handleNavClick);
    s.vid.on('click', NinjaClassApp.handleVideoClick);
    //new
    s.shareFacebook.on('click', NinjaClassApp.handleFacebookShareClick);
    s.shareTwitter.on('click', NinjaClassApp.handleTwitterShareClick);
    //end
    $('.cta-sign-up').on('click', NinjaClassApp.handleSignUpClick);
    NinjaClassApp.createPageTimers();
    NinjaClassApp.setUpKonamiCode();
  },
  handlePlayClick: function(e) {
    e.stopPropagation();
    NinjaClassApp.triggerGAEvent('Clicked Button', 'Play for FREE Button');
  },
  handleSignUpClick: function(e) {
    e.stopPropagation();
    NinjaClassApp.triggerGAEvent('Clicked Link', 'Sign up now');
  },
  /* new tracking events */
  handleFacebookShareClick: function(e) {
    e.stopPropagation();
    NinjaClassApp.triggerGAEvent('Clicked Button', 'Share This Facebook Button');
  },
  handleTwitterShareClick: function(e) {
    e.stopPropagation();
    NinjaClassApp.triggerGAEvent('Clicked Button', 'Share This Twitter Button');
  },
  /* end new tracking events */
  createPageTimers: function() {
    // 1 minute on page
    setTimeout(function() {
      NinjaClassApp.triggerGAEvent('Spent 1 minute on page', ' ');
    }, 1 * 60 * 1000);
    // 2 minutes on page
    setTimeout(function() {
      NinjaClassApp.triggerGAEvent('Spent 2 minutes on page', ' ');
    }, 2 * 60 * 1000);
    // 5 minutes on page
    setTimeout(function() {
      NinjaClassApp.triggerGAEvent('Spent 5 minutes on page', ' ');
    }, 5 * 60 * 1000);
  },
  setUpKonamiCode: function() {
    // NinjaClassApp.s.easter_egg = new Konami(function() {NinjaClassApp.createParticleSystem()});
  },
  ...
  handleVideoClick: function(e) {
    ...
    NinjaClassApp.triggerGAEvent('Clicked Button', 'Video Play Button ' + this.id);
    e.preventDefault();
  },
  ...
  handleNavClick: function(e) {
    ...
    if (external) {
      if (target == '/') {
        NinjaClassApp.triggerGAEvent('Clicked Button', 'Back to Tera - Menu');
      } else {
        NinjaClassApp.triggerGAEvent('Clicked Button', 'Get Started - Menu');
      }
    } else {
      e.preventDefault();
      ...
      complete: function() {
        ...
        NinjaClassApp.triggerGAEvent('Clicked Internal Navigation - ', 'section (' + target + ')' );
  ...
  triggerGAEvent: function(action, label ) {
    ...
    if (_gaq) {
      if (label != ' ') {
        _gaq.push(['_trackEvent', 'Landing Pages', action, NinjaClassApp.s.url + ' - ' + label]);
      } else {
        _gaq.push(['_trackEvent', 'Landing Pages', action, NinjaClassApp.s.url]);
      }
    }
  }
}
