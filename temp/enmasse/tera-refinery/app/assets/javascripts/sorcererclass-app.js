...
var SorcererClassApp = {
  ...
  init: function(settings) {
    var s = SorcererClassApp.s;
    s = $.extend(s, settings);
    SorcererClassApp.bindEvents();
  },
  bindEvents: function() {
    var s = SorcererClassApp.s;
    // set up the scrolling menu waypoint
    $(window).waypoint(function() {
      SorcererClassApp.toggleMainNav();
    }, {offset: s.navOffset});
    // set up new world toggle waypoint
    $(s.intro).waypoint(function(direction) {
      SorcererClassApp.toggleSelectedMenuItem('#intro', direction);
      // only record event on scroll and not click
      if(s.navnotscroll == false) {
        SorcererClassApp.triggerGAEvent('Reached Page Section', 'Hero top of Page');
      }
    $('#main-nav a').removeClass('active');
    });
    // set up Brawler class toggle waypoint
    $(s.highlights).waypoint(function(direction) {
      SorcererClassApp.toggleSelectedMenuItem('#highlights', direction);
      // only record event on scroll and not click
      if(s.navnotscroll == false) {
        SorcererClassApp.triggerGAEvent('Reached Page Section', 'Class Overview');
      }
         //SorcererClassApp.moveBarRight();
      $('#main-nav a').removeClass('active');
      $('#main-nav a#nav-highlights').toggleClass('active');
    });
    $(s.guide).waypoint(function(direction) {
      SorcererClassApp.toggleSelectedMenuItem('#guide', direction);
      // only record event on scroll and not click
      if(s.navnotscroll == false) {
        SorcererClassApp.triggerGAEvent('Reached Page Section', 'Video Guide');
      }
      $('#main-nav a').removeClass('active');
      $('#main-nav a#nav-guide').toggleClass('active');
    });
    $(s.skills).waypoint(function(direction) {
      SorcererClassApp.toggleSelectedMenuItem('#skills', direction);
      // only record event on scroll and not click
      if(s.navnotscroll == false) {
        SorcererClassApp.triggerGAEvent('Reached Page Section', 'Skills and Glyphs');
      }
      $('#main-nav a').removeClass('active');
      $('#main-nav a#nav-skills').toggleClass('active');
    });
    $(s.footer).waypoint(function(direction) {
      SorcererClassApp.triggerGAEvent('Reached Page Section', 'Footer with CTA');
    });
    $(s.ankle).waypoint(function(direction) {
      SorcererClassApp.toggleSelectedMenuItem('.ankle', direction);
      SorcererClassApp.triggerGAEvent('Reached Page Section', 'Ankle');
    });
    // set up click listeners on the menu buttons 
    s.nav.on('click', 'a', SorcererClassApp.handleNavClick);
    s.vid.on('click', SorcererClassApp.handleVideoClick);
    //new
    s.shareFacebook.on('click', SorcererClassApp.handleFacebookShareClick);
    s.shareTwitter.on('click', SorcererClassApp.handleTwitterShareClick);
    //end
    $('.cta-sign-up').on('click', SorcererClassApp.handleSignUpClick);
    SorcererClassApp.createPageTimers();
    SorcererClassApp.setUpKonamiCode();
  },
  handlePlayClick: function(e) {
    e.stopPropagation();
    SorcererClassApp.triggerGAEvent('Clicked Button', 'Play for FREE Button');
  },
  handleSignUpClick: function(e) {
    e.stopPropagation();
    SorcererClassApp.triggerGAEvent('Clicked Link', 'Sign up now');
  },
  /* new tracking events */
  handleFacebookShareClick: function(e) {
    e.stopPropagation();
    SorcererClassApp.triggerGAEvent('Clicked Button', 'Share This Facebook Button');
  },
  handleTwitterShareClick: function(e) {
    e.stopPropagation();
    SorcererClassApp.triggerGAEvent('Clicked Button', 'Share This Twitter Button');
  },
  /* end new tracking events */
  createPageTimers: function() {
    // 1 minute on page
    setTimeout(function() {
      SorcererClassApp.triggerGAEvent('Spent 1 minute on page', ' ');
    }, 1 * 60 * 1000);
    // 2 minutes on page
    setTimeout(function() {
      SorcererClassApp.triggerGAEvent('Spent 2 minutes on page', ' ');
    }, 2 * 60 * 1000);
    // 5 minutes on page
    setTimeout(function() {
      SorcererClassApp.triggerGAEvent('Spent 5 minutes on page', ' ');
    }, 5 * 60 * 1000);
  },
  setUpKonamiCode: function() {
    // SorcererClassApp.s.easter_egg = new Konami(function() {SorcererClassApp.createParticleSystem()});
  },
  ...
  handleVideoClick: function(e) {
    ...
    SorcererClassApp.triggerGAEvent('Clicked Button', 'Video Play Button ' + this.id);
    e.preventDefault();
  },
  ...
  handleNavClick: function(e) {
    ...
    if (external) {
      if (target == '/') {
        SorcererClassApp.triggerGAEvent('Clicked Button', 'Back to Tera - Menu');
      } else {
        SorcererClassApp.triggerGAEvent('Clicked Button', 'Get Started - Menu');
      }
    } else {
      e.preventDefault();
      ...
      complete: function() {
        ...
        SorcererClassApp.triggerGAEvent('Clicked Internal Navigation - ', 'section (' + target + ')' );
  ...
  triggerGAEvent: function(action, label ) {
    ...
    if (_gaq) {
      if (label != ' ') {
        _gaq.push(['_trackEvent', 'Landing Pages', action, SorcererClassApp.s.url + ' - ' + label]);
      } else {
        _gaq.push(['_trackEvent', 'Landing Pages', action, SorcererClassApp.s.url]);
      }
    }
  }
}
