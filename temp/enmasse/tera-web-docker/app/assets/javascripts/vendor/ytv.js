...
(function(win, doc) {
	'use strict';
	var apiKey = 'AIzaSyC-C9DtihymGiG32HpA6JN_Scaff77Dujg';
			...
			action = {
				...
				endpoints: {
					videoClick: function(e){
						...
						if(target){
							// GA tracking
							var selectedVideo = target.getElementsByClassName('ytv-content')[0].getElementsByTagName('b')[0].innerHTML;
							if(selectedVideo) {
								// can capture video name
								ga('send', 'event', 'TERA Media', 'Clicked Youtube Video - Vid Name', selectedVideo, {'nonInteraction': 1});
							} else {
								// cannot capture video name, pass video id instead
								ga('send', 'event', 'TERA Media', 'Clicked Youtube Video - Vid ID', target, {'nonInteraction': 1});
							}
							// end tracking
