document.addEventListener('DOMContentLoaded', function() {
  ...
  if($('.category-content').length > 0) {
    ...
    if($('.category-content.Masse').length > 0) {
      console.log('enmasse category page');
    }
    ...
    $.ajax({
      url: '/api/v2/help_center/en-us/articles.json?label_names=priority&sort_order=desc',
      ...
      if(data.articles.length > 0) {
        ...
        $.each(data.articles, function() {
          ...
          if($(this)[0].draft == false) {
            bucketArts += '<li><a href="' + $(this)[0].html_url + ' " class="article-list-link">' + $(this)[0].title + '</a></li>';
          }
        });
