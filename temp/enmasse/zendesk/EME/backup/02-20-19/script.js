document.addEventListener('DOMContentLoaded', function() {
  ...
  var extraArticles = function(quantifier, cb) {
      $.ajax({
          type: 'GET',
          url: '/api/v2/help_center/en-us/articles.json?label_names=' + quantifier + '&sort_order=desc',
 ...
 /* function extraArticles(quantifier) {
  ...
  if($('.category-content').length > 0) {
    extraArticles('Account', callbackAll);
    
    if($('.category-content.Kritika').length > 0) {
      //console.log('kritika category page');
      extraArticles('Kritika', callbackKrit);
    }
    if($('.category-content.Closers').length > 0) {
      //console.log('closers category page');
      extraArticles('Closers', callbackClos);
    }
    if($('.category-content.TERA').length > 0) {
      //console.log('tera category page');
      extraArticles('TERA', callbackTera);
    }
  };
